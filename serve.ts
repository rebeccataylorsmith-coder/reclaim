...
// second, SSR for the rest. Run `bun run build` before starting.
...
import { handleApiRequest } from "./src/api/handlers";
  ...
  try {
    Bun.serve({
      ...
      async fetch(req) {
        ...
        // API routes — intercept before static files and SSR
